...
- priceId comes from env: NEXT_PUBLIC_STRIPE_PRICE_INICIAL, NEXT_PUBLIC_STRIPE_PRICE_PROFISSIONAL
  (Or hardcode for now with TODO comment - user will configure)
...
**Update proxy.ts:**
Add /checkout to public routes (users need to access it after signup but before having subscription):

```typescript
const publicRoutes = ['/', '/login', '/signup', '/checkout']
```

Also add logic: if user is authenticated but on /checkout, allow access (they need to complete checkout).
  </action>
  <verify>
1. Run signup flow
