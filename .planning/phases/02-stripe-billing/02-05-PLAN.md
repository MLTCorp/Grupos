...
files_modified:
  ...
  - proxy.ts
autonomous: true
# Scope note: This plan has 3 tasks touching 6 files. This is at the upper limit but kept unified
# because the tasks share the same concern (subscription UI gating) and depend on each other.
# Task 3 is subdivided into PART A/B/C to improve executability.

must_haves:
  truths:
    - "Dashboard layout checks subscription and shows trial badge"
    - "User sees trial countdown as 'Trial: Xd' badge in dashboard header (BILL-03 - header placement satisfies requirement as it's visible on every dashboard page)"
    - "User in grace period sees 'Renove em Xd' warning badge"
    - "User can access billing page to see subscription status and detailed countdown"
...
<task type="auto">
  ...
  <action>
This task creates 3 files in sequence. Execute in this order:

**PART A: Create app/dashboard/billing/page.tsx:**
...
**PART B: Create app/blocked/page.tsx:**
