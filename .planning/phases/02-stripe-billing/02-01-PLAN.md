...
must_haves:
  truths:
    - "System can process subscription payments (Stripe SDK initialized and configured)"
    - "System can store and query customer billing information (database schema deployed)"
    - "Webhooks can write subscription data without user session (service client available)"
  artifacts:
    - path: "lib/stripe.ts"
