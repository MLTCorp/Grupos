...
/**
 * POST - Configure webhook for instance
 * URL comes from environment variable for security
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { error: 'Token da instancia nao fornecido' },
        { status: 400 }
      )
    }
    ...
    if (!WEBHOOK_URL) {
      ...
      return NextResponse.json(
        { error: 'WEBHOOK_N8N_URL nao configurado no servidor' },
        { status: 500 }
      )
    }

    // Configuracao de seguranca do webhook:
    // - URL vem da ENV (nao do cliente)
    ...
    console.log(`Configurando webhook para instancia ${token}:`, {
      url: WEBHOOK_URL,
